...
def sc_sisletter(w: World):
    ...
    return w.scene("姉の手紙",
            ...
            mika.explain("最近絵画教室に通い始めた彼女が描いたもので、",
                "何を考えて描いたのか相変わらず自分にはとても理解が及ばないと感じたが、",
                "人生に悩んであちこちの病院に通っていた頃に比べれば、文面からは随分元気になったように思えた", "&"),
            mika.do("その教室の先生が若くてかっこいい、というのが一番の要因だろう"),
            _.do("絵葉書を見つめながら$meはまだこの時、それを微笑ましいと思っていたのだ"),
            camera=w.mika,
            area=w.Tokyo,
            stage=w.on_street,
...
def sc_artclass(w: World):
    ...
    return w.scene("絵画教室",
            ...
            ane.talk("あれは……？"),
            ane.do("一番奥の部屋だった",
                "真っ白で天井の高いそこにはたった一つしか額がなく、それを見上げて$Sは動きを止める"),
            ane.talk("何も、描かれてない？"),
...
def sc_vanish(w: World):
    ...
    return w.scene("姉が消えた",
            ...
            mika.do("呼鈴が何度も空しく響くだけで応答はなかった",
                "$meは仕方なく合鍵で玄関のドアを開け、中に入ってみることにした"),
            mika.do("部屋はとても綺麗に片づいていた", "&"),
            mika.do("誰の姿もなく、それでいてコンロには湯気を昇らせる味噌汁の鍋があった"),
            mika.talk("おねえ、ちゃん？"),
            mika.do("寝室には一枚の大きな絵がイーゼルに立てかけられていたが、", "&"),
            mika.do("そこには何かを見て微笑んでいる姉の姿が描かれていた"),
            w.symbol("（了）"),
            camera=w.mika,
            )
